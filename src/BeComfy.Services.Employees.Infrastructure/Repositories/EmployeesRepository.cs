...
namespace BeComfy.Services.Employees.Infrastructure.Repositories
{
    public class EmployeesRepository : IEmployeesRepository
    {
        ...
        public async Task<Employee> GetAsync(Expression<Func<Employee, bool>> predicate)
            => await _collection.GetAsync(predicate);

        public async Task<List<Employee>> GetManyAsync(Expression<Func<Employee, bool>> predicate, int amount)
            => await _collection.GetManyAsync(predicate, amount);
